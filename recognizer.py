...
class DollarRecognizer:
    ...
    def recognize(self, points, useProtractor):
        ...
        u = -1
        b = float('inf')
        for i, strokes in enumerate(self.Unistrokes):
            if (useProtractor):
                d = OptimalCosineDistance(strokes.points, candidate.points)
            else:
                d = DistanceAtBestAngle(candidate.points, strokes, -AngleRange, +AngleRange, AnglePrecision)
            if (d < b):
                    b = d
                    u = i
